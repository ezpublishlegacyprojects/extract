		...
		<fieldset>
			...
			<div class="block">
				...
				<label>{'Depth'|i18n('design/standard/extract')}</label>
					...
					{section loop=$types}
						<option value="{$:item.id}" {section show=$:item.id|eq($Type)} selected{/section}>{$:item.name}</option>
						{/section}
					</select>
				</div>
				<div class="element">
				...
				<label>{'Mainnode only'|i18n('design/standard/extract')}</label>
					...
					{section loop=$types}
						<option value="{$:item.id}" {section show=$:item.id|eq($Mainnodeonly)} selected{/section}>{$:item.name}</option>
						{/section}
					</select>
				</div>
			</div>
			...
			<div class="block">
				...
				<div class="element">
					...
					<select name="LineSeparator">
						...
						<option value="{$:item.id}" {section show=$:item.id|eq($LineSeparator)} selected{/section}>{$:item.name}</option>
						{/section}
					</select>
				</div>
				<div class="element">
				...
				<table class="list" cellspacing="0">
					...
					<tr>
						...
						<td>
							...
							<input name="Attributes[{$:index}][name]" type="hidden" value="{$:item.name}">
							{$:item.name}
						</td>
